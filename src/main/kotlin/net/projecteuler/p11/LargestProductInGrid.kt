...
object LargestProductInGrid {
    ...
    fun getLargesProductInAGrid() : Int {
        var maxProduct = 0
        for (i in 0..20 - 4)
            for (j in 0..20 - 4) {
                val arr = arrayOf(
                    intArrayOf(number[i+0][j], number[i+0][j+1], number[i+0][j+2], number[i+0][j+3]),
                    intArrayOf(number[i+1][j], number[i+1][j+1], number[i+1][j+2], number[i+1][j+3]),
                    intArrayOf(number[i+2][j], number[i+2][j+1], number[i+2][j+2], number[i+2][j+3]),
                    intArrayOf(number[i+3][j], number[i+3][j+1], number[i+3][j+2], number[i+3][j+3])
                )
                maxProduct = maxOf(maxProduct, getProduct(arr))
            }

        return maxProduct
    }

    private fun getProduct(array: Array<IntArray>) : Int {
        var maxProduct = 0

        println("{${array[0][0]}, ${array[0][1]}, ${array[0][2]}, ${array[0][3]}}")
        println("{${array[1][0]}, ${array[1][1]}, ${array[1][2]}, ${array[1][3]}}")
        println("{${array[2][0]}, ${array[2][1]}, ${array[2][2]}, ${array[2][3]}}")
        println("{${array[3][0]}, ${array[3][1]}, ${array[3][2]}, ${array[3][3]}}")

        var product = 0

        product = array[0][0] * array[0][1] * array[0][2] * array[0][3]
        maxProduct = maxOf(maxProduct, product)
        product = array[1][0] * array[1][1] * array[1][2] * array[1][3]
        maxProduct = maxOf(maxProduct, product)
        product = array[2][0] * array[2][1] * array[2][2] * array[2][3]
        maxProduct = maxOf(maxProduct, product)
        product = array[3][0] * array[3][1] * array[3][2] * array[3][3]
        maxProduct = maxOf(maxProduct, product)

        product = array[0][0] * array[1][0] * array[2][0] * array[3][0]
        maxProduct = maxOf(maxProduct, product)
        product = array[0][1] * array[1][1] * array[2][1] * array[3][1]
        maxProduct = maxOf(maxProduct, product)
        product = array[0][2] * array[1][2] * array[2][2] * array[3][2]
        maxProduct = maxOf(maxProduct, product)
        product = array[0][3] * array[1][3] * array[2][3] * array[3][3]
        maxProduct = maxOf(maxProduct, product)

        product = array[0][0] * array[1][1] * array[2][2] * array[3][3]
        maxProduct = maxOf(maxProduct, product)

        product = array[3][0] * array[2][1] * array[1][2] * array[0][3]
        maxProduct = maxOf(maxProduct, product)
